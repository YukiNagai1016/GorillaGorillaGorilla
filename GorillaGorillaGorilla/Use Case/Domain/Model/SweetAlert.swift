...
open class SweetAlert: UIViewController {
    ...
    fileprivate func resizeAndRelayout() {
        ...
        if self.subTitleTextView.text.isEmpty == false {
            ...
            let rect = subtitleString.boundingRect(with: CGSize(width: width, height: 0.0), options: NSStringDrawingOptions.usesLineFragmentOrigin, attributes: [kCTFontAttributeName as NSAttributedString.Key:subTitleTextView.font!], context: nil)
